...
| **RAG script** | 1) `AI_DEV_SUITE_RAG_SCRIPT` env (absolute path); 2) `rag` in PATH; 3) Walk up from cwd looking for `ai-dev-suite/rag/rag.py`, `rag/rag.py`, or `rag.py` in any ancestor dir |
