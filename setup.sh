...
if [[ "$OS" != "windows" ]] && [[ "$OS" != "unknown" ]]; then
  ...
  if command -v mix >/dev/null 2>&1; then
    ...
    ELIXIR_TUI=""
    [[ -d "setup-main/ai-dev-suite/elixir_tui" ]] && ELIXIR_TUI="setup-main/ai-dev-suite/elixir_tui"
    if [[ -n "$ELIXIR_TUI" ]] && [[ -f "$ELIXIR_TUI/mix.exs" ]]; then
      cd "$ELIXIR_TUI"
      ...
      echo -e "  ${DIM}ai-dev-suite elixir_tui not found in zerwiz/setup.${RESET}"
    fi
  fi
fi
